...
/// `expected`가 None(미설정)이면 검증을 건너뛴다.
pub async fn api_key_middleware(
    expected: Arc<Option<String>>,
    req: Request,
    next: Next,
) -> Response {
    if let Some(expected_key) = expected.as_ref() {
        let provided = req
            .headers()
            .get("x-api-key")
...
impl ConnectionTracker {
    ...
    pub fn acquire(self: &Arc<Self>, ip: IpAddr) -> Result<ConnectionGuard, &'static str> {
        // 전체 연결 수 확인
        let prev = self.total.fetch_add(1, Ordering::Relaxed);
...
pub fn mask_card_no(s: &str) -> String {
    ...
    let masked = s.len().saturating_sub(10);
    format!("{}{}{}", &s[..6], "*".repeat(masked), &s[s.len() - 4..])
}
