...
pub struct Config {
    ...
    /// HTTP API 키 (X-API-Key 헤더). 환경변수 EASYPOS_SECRET_KEY. 필수값 — 미설정 시 서버 시작 거부.
    pub api_key: String,
...
impl Config {
    ...
    pub fn from_env() -> Self {
        Self {
            ...
            api_key: std::env::var("EASYPOS_SECRET_KEY")
                .ok()
                .filter(|s| !s.is_empty())
                .expect("EASYPOS_SECRET_KEY 환경변수가 필수입니다 — X-API-Key 인증에 사용됩니다"),
